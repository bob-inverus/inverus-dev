# inVerus Installation Guide

inVerus is a free, open-source AI chat app with multi-model support. This guide covers how to install and run inVerus on different platforms, including Docker deployment options.

![inVerus screenshot](./public/cover_inVerus.webp)
...
inVerus supports BYOK functionality, allowing users to securely store and use their own API keys for AI providers. To enable this feature, you need to configure an encryption key for secure storage of user API keys.
...
Ollama allows you to run AI models locally on your machine. inVerus has built-in support for Ollama with automatic model detection.
...
### inVerus + Ollama Integration

inVerus automatically detects all models available in your Ollama installation. No additional configuration is needed!

**Features:**

- **Automatic Model Detection**: inVerus scans your Ollama instance and makes all models available
...
By default, inVerus connects to Ollama at `http://localhost:11434`. This works for local installations.
...
inVerus includes a settings interface where you can:
...
For a complete Docker setup with both inVerus and Ollama:
...
docker run -d -v ollama:/root/.ollama -p 11434:11434 --name ollama ollama/ollama
docker run -p 3000:3000 -e OLLAMA_BASE_URL=http://ollama:11434 inVerus
```
...
1. Refresh the models list in inVerus settings
2. Check Ollama has models: `ollama list`
3. Restart inVerus if models were added after startup
...
inVerus requires Supabase for authentication and storage. Follow these steps to set up your Supabase project:
...
For a complete setup with both inVerus and Ollama running locally, use the provided `docker-compose.ollama.yml`:

```bash
# Start both inVerus and Ollama services
...
- **Proper networking** between inVerus and Ollama
- **Volume persistence** for Ollama models

The Ollama service will be available at `http://localhost:11434` and inVerus will automatically detect all available models.
...
The easiest way to deploy inVerus is using Vercel:
...
You can customize various aspects of inVerus by modifying the configuration files:
